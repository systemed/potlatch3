package net.systemeD.halcyon {
    ...
    public class TileSet extends Sprite {
		...
		public function setDimming(dim:Boolean):void { _isDim = dim; updateAlpha(_map.scale); }
		public function getDimming():Boolean { return _isDim; }
		private function updateAlpha(z:int):void { alpha = (_isDim && z>=_map.MINSCALE_DATA) ? 0.5 : 1; }
		...
		/** Set zoom scale (no update triggered). */
			...
			for (var tx:int=tile_l; tx<=tile_r; tx++) {
				for (var ty:int=tile_t; ty<=tile_b; ty++) {
					if (!tiles[_map.scale+','+tx+','+ty]) { 
						...
						loader.x=_map.lon2coord(tile2lon(tx,_map.scale));
						loader.y=_map.lat2coord(tile2lat(ty,_map.scale));
		...
		// baselon/basey has changed, so update all tiles
		// we could probably recalculate all origins (do a .split as per purgeTiles) but this will do for now - the tiles should be cached anyway
		...
		private function tileURL(tx:int,ty:int,tz:uint):String {
			...
			switch (sch.toLowerCase()) {
				...
				case 'bing':
					...
					t=base.replace('{quadkey}',u); break;

				default:
					if (base.indexOf('{x}')>-1) {
						t=base.replace('{zoom}',_map.scale).replace('{z}',_map.scale).replace('{x}',tx).replace('{y}',ty).replace('{-y}',tmsy);
					} else if (base.indexOf('$x')>-1) {
						t=base.replace('$z',_map.scale).replace('$x',tx).replace('$y',ty).replace('$-y',tmsy);
					} else {
						t=base.replace('!',_map.scale).replace('!',tx).replace('!',ty);
					}
		...
		private function tile2lon(t:int,z:int):Number {
			return (t/Math.pow(2,z)*360-180);
		}
		private function tile2lat(t:int,z:int):Number { 
			var n:Number=Math.PI-2*Math.PI*t/Math.pow(2,z);
