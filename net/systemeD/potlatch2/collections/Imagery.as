...
//		private static const INDEX_URL:String="https://josm.openstreetmap.de/maps?format=potlatch";
		...
		private function onImageryIndexLoad(event:Event):void {
			collection = P3.parseJSON( String(event.target.data) ) as Array;
			...
			collection.forEach(function(bg:Object, index:int, array:Array):void {
				...
				if (bg.attribution && bg.attribution.data_url) {
					...
					P3.fetch(bg.attribution.data_url, function(data:String, success:Boolean, err:String):void {
						if (success) onAttributionLoad(data,bg);
					});
				}
			});
		...
		public function onAttributionLoad(data:String, bg: Object):void {
